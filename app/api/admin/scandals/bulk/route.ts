...
export async function POST(req: NextRequest) {
  ...
  if (action === 'delete') {
    try {
      const result = await prisma.scandal.deleteMany({ where: { id: { in: ids as string[] } } })
      return NextResponse.json({ count: result.count })
    } catch {
      return NextResponse.json(
        { error: 'One or more scandals could not be deleted (they may have linked people)' },
        { status: 409 }
      )
    }
  }

  const result = await prisma.scandal.updateMany({
